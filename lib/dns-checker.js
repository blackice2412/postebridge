...
export async function runDnsCheck({ hostname, types, expected = {} }) {
  ...
  const checks = [];
  await Promise.all(recordTypes.flatMap((type) => DNS_RESOLVERS.map(async (resolver) => {
    const result = await queryType(resolver, cleanHost, type);
    checks.push({ resolver: resolver.name, resolverId: resolver.id, region: resolver.region, server: resolver.servers[0], type, ...result, match: compareExpected(type, result.values, expected[type]) });
  })));
  checks.sort((a, b) => (a.type !== b.type ? a.type.localeCompare(b.type) : a.resolver.localeCompare(b.resolver)));
  const summary = {};
  for (const type of recordTypes) summary[type] = summarizeType(checks.filter((c) => c.type === type));
  return { hostname: cleanHost, checkedAt: new Date().toISOString(), checks, summary, resolvers: DNS_RESOLVERS.length };
}
